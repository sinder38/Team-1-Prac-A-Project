...
It works like a fifth agent: parse the old prediction, compare it with
actuals, then suggest small weight changes for the next sprint.
"""
...
WEIGHT_REASONS = {
    "almanac": (
        "Seasonality stays useful, but delta did not show it "
        "should dominate."
    ),
    "macro": (
        "Macro stays important because weekly moves can still depend on "
        "rates, oil, and event risk."
    ),
    "technical": (
        "Technical gets a range-check boost because direction was right "
        "but one range was too tight."
    ),
    "llm": "LLM weight stays stable until we have more weekly history.",
    "human_score": (
        "Human Score gets a small boost because the final direction call "
        "worked."
    ),
}
DELTA_TABLE_HEADER = (
    "| Asset | Predicted direction | Predicted range | Confidence | "
    "Actual move | Actual direction | Direction correct? | Range hit? | "
    "Error % |"
)
DELTA_TABLE_SEPARATOR = (
    "| --- | --- | ---: | --- | ---: | --- | --- | --- | ---: |"
)
WEIGHT_TABLE_HEADER = "| Agent | Current weight | Suggested weight | Reason |"
WEIGHT_TABLE_SEPARATOR = "| --- | ---: | ---: | --- |"
...
class DeltaAgent:
    """Wrapper around DeltaEngine that acts like the fifth agent."""
    ...
    def run(
        ...
        prediction_path = (
            prediction_path or self._prediction_path(prediction_week)
        )
...
def score_asset(prediction: PredictionRow, actual: ActualRow) -> DeltaRow:
    ...
    range_hit = (
        prediction.range_low <= actual.actual_move <= prediction.range_high
    )

    # If the actual move is outside the range, error is the distance
    # to the nearest edge.
    if range_hit:
        error_percent = 0.0
...
def suggest_weight_adjustments(rows: list[DeltaRow]) -> list[WeightAdjustment]:
    ...
    if direction_hits == total and range_hits < total:
        suggested["technical"] += WEIGHT_STEP
        suggested["human_score"] += WEIGHT_STEP
        suggested["almanac"] -= WEIGHT_STEP
        suggested["llm"] -= WEIGHT_STEP
    elif direction_hits < total:
        suggested["human_score"] += WEIGHT_STEP
        suggested["macro"] += WEIGHT_STEP
        suggested["llm"] -= WEIGHT_STEP
        suggested["almanac"] -= WEIGHT_STEP
...
def build_prescription(
    ...
    if wrong_directions:
        target = _join_assets(wrong_directions)
        return (
            "Next sprint should review direction logic for "
            f"{target} before locking the final call."
        )
    if missed_ranges:
        changed = [
            item
        ...
        changed_text = ", ".join(
            (
                f"{item.agent} {item.current_weight:.2f}->"
                f"{item.suggested_weight:.2f}"
            )
            for item in changed
        )
        target = _join_assets(missed_ranges)
        return (
            f"Direction was right, but {target} moved outside the range. "
            "Next sprint should widen range checks and use these draft "
            f"weights: {changed_text}."
        )
    return (
        "Direction and ranges were both strong. Keep the current weights, "
        "but keep tracking the next result before making a bigger change."
    )
...
def render_delta_markdown(report: DeltaReport) -> str:
    ...
    weights = "\n".join(
        _render_weight_row(row) for row in report.weight_adjustments
    )
    ...
    lines = [
        ...
        "",
        (
            f"This file compares the locked {report.prediction_week} "
            f"prediction with the matching {report.actuals_week} actuals. "
            "The goal is simple: check whether the team got the direction "
            "right and how far the actual move was from the predicted "
            "range."
        ),
        "",
        "## Delta table",
        ...
        "## Summary",
        "",
        f"- Direction accuracy: {report.direction_correct_count} / {total}",
        f"- Range accuracy: {report.range_hit_count} / {total}",
        ...
        "",
        (
            "This is the Delta Engine's first draft of how the next sprint "
            "weights could change. It is not a final team decision; it is a "
            "structured starting point for the next prediction cycle."
        ),
        "",
        WEIGHT_TABLE_HEADER,
        WEIGHT_TABLE_SEPARATOR,
...
def _render_table_row(row: DeltaRow) -> str:
    fields = [
        ...
        "Y" if row.direction_correct else "N",
        "Y" if row.range_hit else "N",
...
def _build_short_note(report: DeltaReport) -> str:
    ...
    if report.direction_correct_count == total:
        direction_note = (
            "The team got the broad direction right across all three "
            "indexes."
        )
    elif report.direction_correct_count == 0:
        direction_note = (
            "The team missed the direction across all three indexes."
        )
    else:
        direction_note = (
            f"The team got {report.direction_correct_count} of {total} "
            "directions right."
        )
    ...
    if missed:
        ...
        range_note = (
            "All three actual moves landed inside the predicted ranges."
        )

    return (
        f"{direction_note} {range_note} My main takeaway is that the "
        "direction call was useful, but the range still needs checking when "
        "one index moves more strongly than the others."
    )
