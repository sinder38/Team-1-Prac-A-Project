...
> ✅ Implemented or usable &nbsp;·&nbsp; 🟡 Active integration or review &nbsp;·&nbsp; ⏳ Planned
Component	Status	Current State
Structured weekly prediction workflow	✅	The team produces SPX, NDX, and IWM direction, percentage range, confidence, reasoning, and invalidation conditions.
Automated agent pipeline	✅	Almanac, Technical, Macro, and Evidence stages can run from the Python pipeline.
Macro Agent automation	✅	News and macro inputs no longer require regular manual entry. Free APIs and transformed HTML data replace several paid sources.
Evidence Agent and generated charts	✅	The pipeline can fetch actual market data, create evidence artefacts, and generate chart images.
Multi-LLM synthesis	✅	Four models receive the same evidence and prompt so their conclusions can be compared consistently.
Flask backend API	✅	Stage execution and saved-artifact access are exposed through backend server routes.
React dashboard	✅	The dashboard is usable end-to-end in demo mode and includes pipeline, agents, charts, logs, calibration, and review screens.
Live frontend–backend connection	🟡	Integration work has started and server endpoints exist. Some frontend API adapters still use bundled example data and require final replacement.
Delta Engine	🟡	Core comparison logic, tests, and pipeline integration have been developed. Full merge, API exposure, and dashboard integration remain active work.
Human Score pipeline integration	🟡	The Human Score format exists and is used manually. Automated persistence and pipeline integration are still being completed.
Historical views and richer graphs	🟡	The frontend supports saved-week and chart concepts. Further historical-data integration and visual refinement are in progress.
Local LLM test provider	🟡	Local Ollama support has been added for more reliable development and testing without depending entirely on external API availability.
CI, type checks, and tests	✅	GitHub Actions, Pyright, pytest, frontend build checks, and pull-request review are part of the development workflow.
---
Weekly Intelligence Cycle
Each sprint follows the same evidence-to-learning cycle:
Select the prediction window after the US market closes.
Fetch market and economic data for three target indices and six supporting assets.
Run the analysis stages for Almanac, Macro, Technical, and Evidence.
Query four LLMs with an identical synthesis prompt.
Compare model outputs to identify agreement, disagreement, missing evidence, and uncertainty.
Apply human judgement through the Human Score review.
Lock the prediction for SPX, NDX, and IWM.
Publish the artefacts through GitHub and the project interface.
Evaluate the previous prediction through the Delta Engine.
Record calibration and learning for the next sprint.
---
...
address one clear feature, fix, or sprint artefact;
include meaningful tests where appropriate;
pass type checking and automated checks;
avoid placeholder tests that pass without validating behaviour;
resolve merge conflicts before final review;
explain how the change was tested; and
...
---
Contributing
See CONTRIBUTING.md for the Git workflow, commit-message format, and coding conventions.
